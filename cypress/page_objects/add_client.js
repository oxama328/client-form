...
class AddClient {
  ...
  source() {
    ...
    cy.get('#rc_select_4').scrollIntoView().click({ force: true });
    cy.wait(500); // Wait for dropdown animation
    // Select the first available .infinite-row-scoped option
    cy.get('.ant-select-dropdown:visible .infinite-row-scoped', { timeout: 10000 })
      .first()
      .should('be.visible')
      .click({ force: true });
  }
